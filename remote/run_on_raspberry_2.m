...
board = setup_board();
build(modelName)
deploy(rpi, modelName )

cd ..

r  = raspberrypi;
running = isModelRunning(r,modelName);
runModel(r,modelName);
...
%cd('C:\Users\vic-b\Documents\Victors\Job\Biosim\Learning_Ws\remote\Execution_Data')
filename = sprintf('/home/pi/%s*',modelName)
destination = sprintf('%s*',modelName)
cd remote
cd Execution_Data
getFile(rpi,filename)
cd ..
cd ..

%cd('C:\Users\vic-b\Documents\Victors\Job\Biosim\Learning_Ws')

while isModelRunning(r,modelName) ~= 0 
    stopModel(r,modelName);
end
running = isModelRunning(r,modelName);
